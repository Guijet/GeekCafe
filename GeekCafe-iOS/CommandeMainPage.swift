...
class CommandeMainPage: UIViewController {
    ...
    func fillFakeItems(){
        arrayItems.append(ItemCommande(id: 1, type: "Breuvage", image: UIImage(named:"coffee")!))
        arrayItems.append(ItemCommande(id: 2, type: "Patisserie", image: UIImage(named:"muffin")!))
        arrayItems.append(ItemCommande(id: 3, type: "Crêpe", image: UIImage(named:"crepe")!))
        arrayItems.append(ItemCommande(id: 4, type: "Fondue", image: UIImage(named:"fondue")!))
    }
    ...
    func fillScrollView(){
        if(arrayItems.count > 0){
            ...
            for x in arrayItems{
                ...
                imageItem.frame = CGRect(x: rw(16), y: rh(8), width: rw(100), height: rw(100))
                imageItem.image = x.image
                buttonItem.addSubview(imageItem)
                
                let titleItem = UILabel()
                titleItem.createLabel(frame: CGRect(x: 0, y: rh(99), width: buttonItem.frame.width, height: rh(25)), textColor: Utility().hexStringToUIColor(hex: "#6CA642"), fontName: "Lato-Regular", fontSize: rw(17), textAignment: .center, text: x.type)
